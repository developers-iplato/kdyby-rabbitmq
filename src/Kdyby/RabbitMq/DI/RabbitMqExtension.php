...
class RabbitMqExtension extends Nette\DI\CompilerExtension
{
	 ...
	 * @var array
	 */
    protected $configuredServices = array(
        'producers' => array(),
        'consumers' => array(),
        'rpcClients' => array(),
        'rpcServers' => array(),
	...
	public function loadConfiguration()
	{
        ...
        $builder->addDefinition($this->prefix('registry'))->setClass('Kdyby\RabbitMq\Registry', array(
            $this->configuredServices['producers'],
            $this->configuredServices['consumers'],
            $this->configuredServices['rpcClients'],
            $this->configuredServices['rpcServers'],
        ));
	...
	protected function loadProducers($producers)
	{
		...
		foreach ($producers as $name => $config) {
            ...
            $this->configuredServices['producers'][$name] = $serviceName;
	...
	protected function loadConsumers($consumers)
	{
		...
		foreach ($consumers as $name => $config) {
            ...
            $this->configuredServices['consumers'][$name] = $serviceName;
		}
	}
	...
	protected function loadRpcClients($clients)
	{
		...
		foreach ($clients as $name => $config) {
            ...
            $this->configuredServices['rpcClients'][$name] = $serviceName;
		}
	}
	...
	protected function loadRpcServers($servers)
	{
		...
		foreach ($servers as $name => $config) {
            ...
            $this->configuredServices['rpcServers'][$name] = $serviceName;
		}
	}
